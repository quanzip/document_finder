...
export class StompService implements OnDestroy {
    ...
    private resolveConPromise: ((...args: any[]) => void) | undefined;
    public queuePromises = [];

    private disconnectPromise: any;
    private resolveDisConPromise: ((...args: any[]) => void) | undefined;
    ...
    public startConnect(): Promise<{}> {
        ...
        if (this.config.debug) {
            this.stomp.debug = function (str: any) {
                console.log(str);
            };
        } else {
    ...
    public subscribe(destination: string, callback: any, headers?: Object) {
        headers = headers || {};
        return this.stomp.subscribe(destination, function (response: any) {
    ...
    public disconnect(): Promise<{}> {
        this.stomp.disconnect(() => { // @ts-ignore
    ...
    public after(name: string): Promise<{}> {
        ...
        if (!this.queuePromises[name + 'promice']) {
            // @ts-ignore
            this.queuePromises[name + 'promice'] = new Promise((resolve) => this.queuePromises[name] = resolve);
        }
        // @ts-ignore
